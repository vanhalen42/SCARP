...
class TFN(torch.nn.Module):
    ...
    def __init__(self, sphere_samples = 64, bn_momentum = 0.75, mlp_units = [[32, 32], [64, 64], [128, 256]], l_max = [3, 3, 3], l_max_out = [3, 3, 3], num_shells = [3, 3, 3]):
        ...
        self.code_mlp = MLP_layer(in_channels = self.mlp_units[-1][-1], units = [128], bn_momentum = self.bn_momentum)
        self.code_layer = MLP(in_channels = 128, out_channels=128, apply_norm = False)
    ...
    def forward(self, x):
        ...
        features = self.code_mlp(F)
        features = self.code_layer(features)
        return F, features
